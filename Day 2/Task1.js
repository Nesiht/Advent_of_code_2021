...
const calculatePosition = (data) => {
  for (let i in data) {
    ...
    switch (data[i].direction[0]) {
      case 'forward' : {horizontalPosition += data[i].value} break
      case 'down' : {depthPosition += data[i].value} break
      case 'up': {depthPosition -= data[i].value} break
    }
  }
}
...
fs.readFile('input.txt', 'utf8', (err, data) => {
  ...
  console.log("Horizontal:", horizontalPosition, "Depth:", depthPosition)
